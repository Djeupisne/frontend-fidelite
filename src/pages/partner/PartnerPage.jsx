...
const TEXT_FIELDS = [{ name:'nom', label:'Nom', required:true }, { name:'prenom', label:'Prénom', required:true }, { name:'age', label:'Age', type:'number', required:true }, { name:'ville', label:'Ville', required:true }, { name:'quartier', label:'Quartier', required:true }, { name:'profession', label:'Profession' }, { name:'nomMere', label:'Nom complet de la mère', required:true }, { name:'nomPere', label:'Nom complet du père', required:true }, { name:'nomAine', label:"Nom de l'aîné de la famille", required:true }, { name:'nomBenjamin', label:'Nom du benjamin de la famille', required:true }, { name:'lieuRencontre', label:'Lieu de la rencontre' }];
        ...
        {!editing ? (
          <div className="card">
            <div className="flex items-center gap-5 mb-6 pb-6 border-b border-gray-100">
              {partner?.photo ? <img src={partner.photo} alt={`${partner.prenom} ${partner.nom}`} className="w-20 h-20 rounded-2xl object-cover shadow" /> : <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-secondary-100 to-secondary-200 flex items-center justify-center text-secondary-700 font-bold text-2xl">{getInitials(partner?.nom, partner?.prenom)}</div>}
              <div>
          ...
          <div className="card space-y-5">
            <div className="flex items-center gap-3 pb-4 border-b border-gray-100">
              <div className="w-10 h-10 bg-secondary-100 rounded-xl flex items-center justify-center text-xl">✏️</div>
              <div><h2 className="font-semibold text-gray-900">Modifier les informations</h2><p className="text-xs text-gray-500">Mettez à jour les données du partenaire</p></div>
            </div>
            <div className="flex justify-center"><PhotoUpload label="Photo du partenaire" onChange={setNewPhoto} preview={partner?.photo} /></div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {TEXT_FIELDS.map(f => (
                <div key={f.name}>
                  <label className="label" htmlFor={`pp-${f.name}`}>{f.label}{f.required && <span className="text-red-500 ml-1">*</span>}</label>
                  <input id={`pp-${f.name}`} type={f.type || 'text'} value={form[f.name] || ''} onChange={e => setForm(p => ({...p, [f.name]: e.target.value}))} autoComplete="off" className="input" />
                </div>
              ))}
              <div>
                <label className="label" htmlFor="pp-pays">Pays{<span className="text-red-500 ml-1">*</span>}</label>
                <select id="pp-pays" value={form.pays || ''} onChange={e => setForm(p => ({...p, pays: e.target.value}))} autoComplete="off" className="input">
                  <option value="">Choisir...</option>
                  {PAYS.map(o => <option key={o} value={o}>{o}</option>)}
                </select>
              </div>
              <div>
                <label className="label" htmlFor="pp-religion">Religion</label>
                <select id="pp-religion" value={form.religion || ''} onChange={e => setForm(p => ({...p, religion: e.target.value}))} autoComplete="off" className="input">
                  <option value="">Choisir...</option>
                  {RELIGIONS.map(o => <option key={o} value={o}>{o}</option>)}
                </select>
              </div>
              <div>
                <label className="label" htmlFor="pp-etude">Niveau d'étude</label>
                <select id="pp-etude" value={form.niveauEtude || ''} onChange={e => setForm(p => ({...p, niveauEtude: e.target.value}))} autoComplete="off" className="input">
                  <option value="">Choisir...</option>
                  {ETUDES.map(o => <option key={o} value={o}>{o}</option>)}
                </select>
              </div>
              <div>
                <label className="label" htmlFor="pp-date">Date de la rencontre</label>
                <input id="pp-date" type="date" value={form.dateRencontre || ''} onChange={e => setForm(p => ({...p, dateRencontre: e.target.value}))} autoComplete="off" className="input" />
              </div>
            </div>
