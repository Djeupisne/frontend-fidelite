...
export default function EditProfilePage() {
  ...
  const F = ({ label, name, type = 'text', placeholder }) => (
    <div><label className="label">{label}</label><input {...register(name)} type={type} placeholder={placeholder} className={`input ${errors[name] ? 'input-error' : ''}`} />{errors[name] && <p className="text-red-500 text-xs mt-1">{errors[name]?.message}</p>}</div>
  );
  const S = ({ label, name, options }) => (
    <div><label className="label">{label}</label><select {...register(name)} className="input"><option value="">Choisir...</option>{options.map(o => <option key={o} value={o}>{o}</option>)}</select></div>
  );
  return (
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <F label="Nom de votre mère" name="nomMere" placeholder="Nom complet" />
              <F label="Nom de votre père" name="nomPere" placeholder="Nom complet" />
              <F label="Nom de l'aîné" name="nomAine" placeholder="Nom complet" />
              <F label="Nom du benjamin" name="nomBenjamin" placeholder="Nom complet" />
            </div>
          </div>
